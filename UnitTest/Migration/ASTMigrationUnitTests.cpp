...
void DefaultMigratorMock::LaunchCheckExpiredEvent()
{
...
void DefaultMigratorTest::TearDown()
{
    /*
     * NOTE:
     * Currently for DefaultMigratorTest, CleanUp() has the same
     * effect as TearDown(), but to certain extent, CleanUp()
     * acts as synchronization tool.
     * */
//    mDefaultMigratorMock->Stop();
//    mControlCenterMock->Stop();
}

void DefaultMigratorTest::SetHUBSimulateExpiration(ywb_bool_t val)
{
    HUB* hub = NULL;
    HUBMock* hubmock = NULL;

    mControlCenterMock->GetCurHUB(&hub);
    YWB_ASSERT(hub != NULL);
    hubmock = (HUBMock*)hub;
    hubmock->SetSimulateExpiration(val);
}

void DefaultMigratorTest::SetSubPoolKey(
        SubPoolKey& subpoolkey, ywb_uint32_t oss,
...
void DefaultMigratorTest::SetOBJKey(
        OBJKey& objkey, ywb_uint64_t disk,
        ywb_uint64_t inode, ywb_uint64_t chunk)
{
    objkey.SetStorageId(disk);
    objkey.SetInodeId(inode);
    objkey.SetChunkId(chunk);
...
{
    YWB_ASSERT(item != NULL);
    item->SetSubmittedTime(ywb_time_now());
...
ywb_status_t DefaultMigratorTest::CheckExpired()
{
    return mDefaultMigratorMock->CheckExpired();
}

void DefaultMigratorTest::LaunchCheckExpired()
{
    mDefaultMigratorMock->LaunchCheckExpiredEvent();
}

void DefaultMigratorTest::LaunchMigrationCompleted(OBJKey objkey)
{
    mDefaultMigratorMock->LaunchCompleteMigrationEvent(objkey, YWB_SUCCESS);
}
...
ywb_uint32_t DefaultMigratorTest::GetInflightNum()
{
    return mDefaultMigratorMock->GetInflightNum();
}
...
void MigrationTest::SetUp()
{
    ...
    while(migratornum < 3)
    {
        ...
        ret = migration->RegisterMigrator(migratorname, migrator);
        YWB_ASSERT(YWB_SUCCESS == ret);
        migratornum++;
    }

    ret = migration->StartMigrator("Migrator-0");
    YWB_ASSERT(YWB_SUCCESS == ret);
}

void MigrationTest::ResetAST()
{
    AST* ast = NULL;
...
void MigrationTest::SetHUBSimulateExpiration(ywb_bool_t val)
{
    HUB* hub = NULL;
    HUBMock* hubmock = NULL;
...
void MigrationTest::TearDown()
{
    ...
    while(migratornum < 3)
    {
        memset(migratorname, 0, 16);
        sprintf(migratorname, "%s-%d", "Migrator", migratornum);
        ret = migration->DeRegisterMigrator(migratorname);
        YWB_ASSERT(YWB_SUCCESS == ret);
        migratornum++;
    }
...
ywb_status_t
MigrationTest::Complete(ywb_uint64_t inodeid,
        ywb_status_t err, ywb_bool_t more)
{
    ...
    if(YWB_SUCCESS == ret)
    {
        ...
        ret = migration->Complete(diskkey, arbitrateekey, err, more);
    }
...
ywb_status_t
MigrationTest::Expire(ywb_uint64_t inodeid, ywb_bool_t more)
{
    ywb_status_t ret = YWB_SUCCESS;
    DiskKey diskkey;
    ArbitrateeKey arbitrateekey;
    ...
    if(YWB_SUCCESS == ret)
    {
        ...
        arbitratee->GetOBJKey(&objkey);
        arbitratee->OBJKey2ArbitrateeKey(objkey, &arbitrateekey);
        ret = migration->Expire(diskkey, arbitrateekey, more);
    }
